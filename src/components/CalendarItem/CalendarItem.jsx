...
export default function CalendarItem({ day, weekDays, selectedDay, currentDay, handleDayClick, todosByDate }) {
  ...
  return (
    <li id={date.getDate()} onClick={handleDayClick} className='calendar-item-container'>
      <div className={cls.join(' ')}>
        <p className='week-day-title'>{weekDays[date.getDay()]}</p>
        <h3>{date.getDate()}</h3>
      </div>
      <div className='calendar-item-status-container'>
        { completedTodos > 0 && <div className='calendar-item-status completed'></div> }
        { uncompletedTodos > 0 && <div className='calendar-item-status uncompleted'></div> }
      </div>
    </li>
  )
}
